...
export async function POST(req: Request) {
  try {
    ...
    const { 
      campaignName, adsetName, adName, 
      pageId, postUrl, objective, 
      conversionLocation, performanceGoal, 
      budgetType, budget, duration,
      ageMin, ageMax, gender, location, targeting,
      callToAction 
    } = body;

    const accessToken = process.env.FB_ACCESS_TOKEN || "YOUR_USER_OR_PAGE_ACCESS_TOKEN";
    const adAccountId = process.env.FB_AD_ACCOUNT_ID || "act_YOUR_AD_ACCOUNT_ID";

    // ១. បង្កើត Campaign ទៅកាន់ Meta API
    const campaignRes = await fetch(`https://graph.facebook.com/v18.0/${adAccountId}/campaigns`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: campaignName,
        objective: objective,
        status: 'PAUSED', // ដាក់ PAUSED ទុកសិនដើម្បីឱ្យយើងឆែកមើលមុនពេលបើក Run
    ...
    if (!campaignData.id) {
      return NextResponse.json({ success: false, error: campaignData.error?.message || "Failed to create campaign" }, { status: 400 });
    }
    const campaignId = campaignData.id;

    // ២. បង្កើត Ad Set ទៅកាន់ Meta API
    const dailyOrLifetime = budgetType === 'DAILY' ? { daily_budget: Math.round(Number(budget) * 100) } : { lifetime_budget: Math.round(Number(budget) * 100) };
    ...
    const adsetRes = await fetch(`https://graph.facebook.com/v18.0/${adAccountId}/adsets`, {
      ...
      body: JSON.stringify({
        name: adsetName,
        optimization_goal: performanceGoal === 'CONVERSATIONS' ? 'CONVERSATIONS' : 'LINK_CLICKS',
        billing_event: 'IMPRESSIONS',
        bid_amount: 100,
        campaign_id: campaignId,
        destination_type: conversionLocation === 'MESSAGES' ? 'MESSENGER' : 'WEBSITE',
        ...dailyOrLifetime,
        targeting: {
    ...
    if (!adsetData.id) {
      return NextResponse.json({ success: false, error: adsetData.error?.message || "Failed to create ad set" }, { status: 400 });
    }
    const adsetId = adsetData.id;

    // ៣. បង្កើត Ad Creative និង Ad ភ្ជាប់ជាមួយ Post ស្រាប់
    const creativeRes = await fetch(`https://graph.facebook.com/v18.0/${adAccountId}/adcreatives`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: `${adName} - Creative`,
        object_story_id: postUrl, // Post ID របស់ Page
    ...
    if (!creativeData.id) {
      return NextResponse.json({ success: false, error: creativeData.error?.message || "Failed to create ad creative" }, { status: 400 });
    }
    ...
    if (!adData.id) {
      return NextResponse.json({ success: false, error: adData.error?.message || "Failed to create ad" }, { status: 400 });
    }

    return NextResponse.json({ success: true, message: "Campaign, Ad Set and Ad created successfully on Facebook!" });
